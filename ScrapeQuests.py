...
traders = driver.find_elements(By.XPATH, value="//*[@id='menubar']/ul[2]/li[7]/ul/li/a")
...
for traderName in tradersList:
    ...
    quests = driver.find_elements(By.XPATH, value="//*[@id='content']/ul[1]/li/a")
    for quest in quests:
        questURLs[traderName].append(quest.get_attribute("href"))

spreadsheetsData = ""
questCount = {
    traderName: {"allCount": 0, "validCount": 0} for traderName in tradersList
}
errorQuestPages = []
for traderName in tradersList:
    ...
    for questURL in questURLs[traderName]:
        ...
        driver.get(questURL)
        try:
            questName = driver.find_element(By.XPATH, value="//*[@id='title']/h1").text
            if "/" in questName:
                questName = questName[questName.index("/") + 1 :]
            kappaRequired = driver.find_element(
                By.XPATH,
                value="//*[@id='content']//td[contains(text(), 'Kappaタスク')]/following-sibling::td[1]",
            ).text
        except:
...
for traderName in tradersList:
    print(traderName + "'s AllQuestPage: " + str(questCount[traderName]["allCount"]))
    print(
        traderName + "'s ValidQuestPage: " + str(questCount[traderName]["validCount"])
    )
